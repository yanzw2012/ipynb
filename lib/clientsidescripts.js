...
functions.waitForAngular = function(selector, callback) {
  ...
  try {
    angular.element(el).injector().get('$testability').notifyWhenNoOutstandingRequests(callback);
  } catch (e) {
...
functions.findBindings = function(binding, exactMatch, using) {
  ...
  var nodes = angular.element(using).injector().get('$testability').
      findBindings(binding, exactMatch);
...
functions.findByModel = function(model, using) {
  using = using || document;
  return angular.element(using).injector().get('$testability').findModels(model);
};
